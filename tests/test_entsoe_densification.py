...
def document(*periods):
    return (f'<?xml version="1.0" encoding="UTF-8"?>'
            ...
            f'{"".join(periods)}</Publication_MarketDocument>').encode()
...
def fetch(monkeypatch):
    ...
    def run(content, status_code=200):
        monkeypatch.setattr(
            main.requests, "get",
            lambda *a, **kw: FakeResponse(content, status_code))
        return pull_entsoe_data(
            "token", datetime.datetime(2025, 12, 1, tzinfo=UTC),
...
def times(result):
    return [datetime.datetime.fromisoformat(p["startTime"])
...
def test_fills_gaps_left_by_omitted_repeated_prices(fetch):
    ...
    result = fetch(document(period(
        "2025-12-01T00:00Z", "2025-12-01T01:00Z", "PT15M",
        [(1, 100.0), (4, 200.0)])))
...
def test_last_point_holds_until_the_end_of_the_period(fetch):
    ...
    assert times(result)[-1] == datetime.datetime(
        2025, 12, 1, 0, 45, tzinfo=UTC)


def test_expands_hourly_resolution_onto_the_quarter_hour_grid(fetch):
    result = fetch(document(period(
        "2025-12-01T00:00Z", "2025-12-01T02:00Z", "PT60M",
        [(1, 100.0), (2, 200.0)])))
...
def test_expands_daily_resolution(fetch):
    result = fetch(document(period(
        "2025-12-01T00:00Z", "2025-12-02T00:00Z", "P1D", [(1, 100.0)])))
...
def test_mixed_resolutions_produce_one_uniform_grid(fetch):
    ...
    result = fetch(document(
        period("2025-12-01T00:00Z", "2025-12-01T02:00Z", "PT60M",
               [(1, 100.0), (2, 200.0)]),
        period("2025-12-01T02:00Z", "2025-12-01T03:00Z", "PT15M",
...
def test_periods_out_of_order_are_sorted(fetch):
    result = fetch(document(
        period("2025-12-01T01:00Z", "2025-12-01T02:00Z", "PT60M",
               [(1, 200.0)]),
        period("2025-12-01T00:00Z", "2025-12-01T01:00Z", "PT60M",
               [(1, 100.0)])))
...
def test_unknown_resolution_raises_rather_than_asserting(fetch):
    ...
    with pytest.raises(APIError, match="PT5M"):
        fetch(document(period(
            "2025-12-01T00:00Z", "2025-12-01T01:00Z", "PT5M",
            [(1, 100.0)])))
...
def test_prices_are_converted_to_cents_per_kilowatt_hour(fetch):
    ...
    result = fetch(document(period(
        "2025-12-01T00:00Z", "2025-12-01T00:15Z", "PT15M", [(1, 45.67)])))
